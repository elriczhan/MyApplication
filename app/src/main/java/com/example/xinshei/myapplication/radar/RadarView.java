package com.example.xinshei.myapplication.radar;
...
import com.example.xinshei.myapplication.radar.util.AnimeUtil;
import com.example.xinshei.myapplication.radar.util.RotateUtil;
...
public class RadarView extends View {
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        mSubVertexTextColor = typedArray.getColor(R.styleable.RadarView_subvertex_text_color, mRadarLineColor);
        mVertexTextSize = typedArray.getDimension(R.styleable.RadarView_subvertex_text_size, dp2px(16));
        mSubVertexTextSize = typedArray.getDimension(R.styleable.RadarView_vertex_text_size, dp2px(12));
    ...
    private void init() {
        ...
        mRadarOutLinePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mLayerPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mValuePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mVertexTextPaint = new TextPaint(Paint.ANTI_ALIAS_FLAG);
        mSubVertexTextPaint = new TextPaint(Paint.ANTI_ALIAS_FLAG);
    ...
    public List<String> getSubVertexText() {
        return mSubvertexText;
    }

    public void setSubVertexText(List<String> subvertexText) {
        this.mSubvertexText = subvertexText;
        initsubVertexText();
        invalidate();
    }
    ...
    public void setSubVertexTextColor(int vertexTextColor) {
        this.mSubVertexTextColor = vertexTextColor;
        invalidate();
    }
    ...
    private void initsubVertexText() {
        if (mSubvertexText == null || mSubvertexText.size() == 0) {
            mSubvertexText = new ArrayList<>();
            for (int i = 0; i < mMaxVertex; i++) {
                char text = (char) ('A' + i);
                mSubvertexText.add(String.valueOf(text));
            }
        } else if (mSubvertexText.size() < mMaxVertex) {
            int size = mMaxVertex - mSubvertexText.size();
            for (int i = 0; i < size; i++) {
                mSubvertexText.add("");
            }
        }
        mMaxLengthVertexText = Collections.max(mSubvertexText, new Comparator<String>() {
            @Override
    ...
    private void initPaint() {
        ...
        mRadarOutLinePaint.setStrokeWidth(mRadarLineWidth);
        mRadarOutLinePaint.setColor(Color.parseColor("#AEDDDD"));
        mRadarOutLinePaint.setStyle(Paint.Style.STROKE);
        mVertexTextPaint.setColor(mVertexTextColor);
        mSubVertexTextPaint.setColor(mSubVertexTextColor);
        mVertexTextPaint.setTextSize(mVertexTextSize);
        mSubVertexTextPaint.setTextSize(mSubVertexTextSize);
...
//            }
            if (i == mLayer) {
                canvas.drawPath(mRadarPath, mRadarOutLinePaint);
            }
        }
    }
    ...
    private void drawRadarLine(Canvas canvas) {
        for (int i = 1; i <= mMaxVertex; i++) {
            ...
            drawSubVertexText(canvas, i, angleSin, angleCos);
    ...
    private void drawSubVertexText(Canvas canvas, int index, double angleSin, double angleCos) {
        float x = (float) (mPointCenter.x + angleSin * (mRadius + mVertexTextOffset));
        float y = (float) (mPointCenter.y + angleCos * (mRadius + mVertexTextOffset)) + dp2px(20);
        String text = mSubvertexText.get(index - 1);
        float textWidth = mSubVertexTextPaint.measureText(text);
        Paint.FontMetrics fontMetrics = mSubVertexTextPaint.getFontMetrics();
        float textHeight = fontMetrics.descent - fontMetrics.ascent;
        canvas.drawText(text, x - textWidth / 2, y + textHeight / 4, mSubVertexTextPaint);
    }
    ...
    private void drawData(Canvas canvas) {
        for (int i = 0; i < mRadarData.size(); i++) {
            ...
            for (int j = 1; j <= values.size(); j++) {
                ...
                canvas.drawCircle(x, y, dp2px(3), mValuePaint); //点
            }
            mRadarPath.close();
